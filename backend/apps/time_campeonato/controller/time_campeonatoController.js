...
exports.updateTimeCampeonato = async (req, res) => {
  ...
  try {
      ...
      if (result.rowCount === 0) {
          // Responde com erro se nenhum registro foi atualizado
          return res.status(404).json({ message: 'Times_Campeonatos não encontrado.' });
      }

      // Responde com sucesso se o registro foi atualizado
      return res.status(200).json(result.rows[0]);
  } catch (error) {
      console.error('Erro ao atualizar times_campeonatos:', error);
      // Responde com erro genérico em caso de falha
      return res.status(500).json({ message: 'Erro ao atualizar o times_campeonatos.' });
  }
};
...
exports.deleteTimeCampeonato = async (req, res) => {
  ...
  try {
      ...
      // Remove o registro
      await pool.query(`DELETE FROM times_campeonatos WHERE id = $1`, [id]);

      return res.status(200).json({ message: 'Registro deletado com sucesso.' });
  } catch (error) {
      console.error('Erro ao deletar times_campeonatos:', error);
      return res.status(500).json({ message: 'Erro ao deletar o times_campeonatos.' });
  }
};
